# Ice-Cream-Sales-Prediction-Using-Linear-Regression
Predicting ice cream sales profits based on temperature using Simple Linear Regression. The project includes data analysis, visualization, model training, prediction using Scikit-Learn.

## Project Overview

This project uses Simple Linear Regression to analyze the relationship between temperature and ice cream sales profits. The model learns the trend from historical data and predicts expected profit based on temperature.

### Technologies Used
- Python
- Pandas
- NumPy
- Matplotlib
- Scikit-Learn

### Workflow
- Data Loading
- Data Exploration
- Correlation Analysis
- Data Visualization
- Model Training
- Prediction
